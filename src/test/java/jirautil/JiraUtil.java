...
public class JiraUtil {
	...
	/**
	 * This function sets the WebDriver and launches the URL specified in Frameowrk.properties file with property key
	 * BaseURL. <br>
	 * <br>
	 * 
	 * @param projectKey
	 ...
	 *        - Specify the path of the attachment to be added to the new bug. The name of the attachment is same as the
	 *        original name of the image.<br>
	 *        <br>
	 * 
	 * @exception Exception
	...
	public String reporIssue(String summary, String description, String attachmentPath) {
		...
		try {
			// create a issue
			newIssue = jira.createIssue(JIRA_PROJECT, "Bug").field(Field.SUMMARY, summary).field(Field.DESCRIPTION, description).field(Field.PRIORITY, "High").execute();
			...
			ExtentUtil.logger.get().log(Status.INFO, HTMLReportUtil.failStringRedColor("Logged Bug ID: " + newIssue.getKey()));
			return bugID;
	...
	/**
	 * @param ExecutionID:
	 *        ID is the number which we get when the testcase is added to a Testcycle and do inspect element and in the html
	 *        serach for "data-issueid"
	 * @param Comments:
	 *        String
	 * @param result
	 *        : Fail
	...
	public void updateJiraTestResults(String ExecutionID, String Comments, String Result) {
		try {
			...
			// updateExecutions(updateExecutionUri, ZAPI_CLIENT, ZAPI_ACCESS_KEY,
			// executeTestsJSON);
			LogUtil.infoLog(JiraUtil.class, "Updated Results to Jira");
		} catch (Exception e) {
			LogUtil.errorLog(JiraUtil.class, e.getMessage());
		}
	}
	...
	private static Map<String, String> getExecutionsID() {
		...
		try {
			final String getExecutionsUri = API_GET_EXECUTIONS.replace("{SERVER}", ZEPHYR_URL) + JIRA_CYCLE_ID + "?projectId=" + JIRA_PROJECT_ID + "&versionId=-1";
